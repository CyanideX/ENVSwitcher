...
class WeatherApp:
    def __init__(self, root):
        ...
        self.left_header = tk.Label(left_frame, text="Disabled States", bg="#2e2e2e", fg="#e7e7e7")
        self.left_header.pack()

        self.right_header = tk.Label(right_frame, text="Enabled States", bg="#2e2e2e", fg="#e7e7e7")
        self.right_header.pack()

        self.left_listbox = tk.Listbox(left_frame, bg="#3e3e3e", fg="#e7e7e7", selectbackground="#5e5e5e", selectforeground="#e7e7e7")
        self.left_listbox.pack(fill=tk.BOTH, expand=True, pady=10)
        self.left_listbox.bind("<Motion>", self.show_tooltip)

        self.right_listbox = tk.Listbox(right_frame, bg="#3e3e3e", fg="#e7e7e7", selectbackground="#5e5e5e", selectforeground="#e7e7e7")
        ...
        self.targets_button = tk.Button(button_frame, text="Transitions", command=self.open_transitions, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.targets_button.pack(pady=5)

        self.properties_button = tk.Button(button_frame, text="Properties", command=self.open_properties, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.properties_button.pack(pady=5)

        move_button_frame = tk.Frame(button_frame, bg="#2e2e2e")
        move_button_frame.pack(pady=5)

        self.remove_button = tk.Button(move_button_frame, text="<<<", command=self.remove_states, width=button_width_arrows, bg="#4e4e4e", fg="#e7e7e7")
        self.remove_button.pack(side=tk.LEFT, padx=5)

        self.add_button = tk.Button(move_button_frame, text=">>>", command=self.add_states, width=button_width_arrows, bg="#4e4e4e", fg="#e7e7e7")
        self.add_button.pack(side=tk.LEFT, padx=5)

        self.save_button = tk.Button(button_frame, text="Save", command=self.save_states, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.save_button.pack(pady=5)

        self.light_mode_button = tk.Button(button_frame, text="Light Mode", command=self.toggle_light_mode, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.light_mode_button.pack(side=tk.BOTTOM, pady=5)

        self.toggle_tooltips_button = tk.Button(button_frame, text="Enable Tooltips", command=self.toggle_tooltips, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.toggle_tooltips_button.pack(side=tk.BOTTOM, pady=5)

        self.film_grain_button = tk.Button(button_frame, text="Film Grain", command=self.open_film_grain, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
        self.film_grain_button.pack(side=tk.BOTTOM, pady=5)

        self.debug_button = tk.Button(button_frame, text="Debug", command=self.open_global_properties, width=button_width, bg="#4e4e4e", fg="#e7e7e7")
    ...
    def show_tooltip(self, event):
        ...
        if index != -1:
            ...
            if handle_id:
                ...
                label = Label(self.tooltip, text=f"Handle ID: {handle_id}", background="#e7e7e7", relief="solid", borderwidth=1)
                label.pack()
    ...
    def populate_listboxes(self, folder_path):
        ...
        for state in self.data.get('Data', {}).get('RootChunk', {}).get('weatherStates', []):
            ...
            if name in exclusion_set:
                self.right_listbox.itemconfig(tk.END, {'fg': 'grey', 'selectbackground': '#e7e7e7', 'selectforeground': 'grey'})
    ...
    def apply_dark_theme(self, widget):
        ...
        for child in widget.winfo_children():
            if isinstance(child, (tk.Frame, tk.Label, tk.Button, tk.Listbox)):
                if 'fg' in child.configure():
                    child.configure(bg="#2e2e2e", fg="#e7e7e7")
                else:
                    child.configure(bg="#2e2e2e")
                self.apply_dark_theme(child)

    def apply_light_theme(self, widget):
        widget.configure(bg="#eeeeee")
        for child in widget.winfo_children():
            if isinstance(child, (tk.Frame, tk.Label, tk.Button, tk.Listbox)):
                if 'fg' in child.configure():
                    child.configure(bg="#eeeeee", fg="#1a1a1a")
                else:
                    child.configure(bg="#eeeeee")
                self.apply_light_theme(child)


    def toggle_light_mode(self):
        if self.light_mode_button.cget("text") == "Light Mode":
            self.apply_light_theme(self.root)
            self.light_mode_button.configure(text="Dark Mode")
        else:
            self.apply_dark_theme(self.root)
            self.light_mode_button.configure(text="Light Mode")

    def open_transitions(self):
        subprocess.Popen(['python', 'transitions.py', '--theme', 'dark'])

    def open_properties(self):
        subprocess.Popen(['python', 'properties.py', '--theme', 'dark'])

    def open_global_properties(self):
        subprocess.Popen(['python', 'global_properties.py', '--theme', 'dark'])

    def open_film_grain(self):
        subprocess.Popen(['python', 'film_grain.py', '--theme', 'dark'])
